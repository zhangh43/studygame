...
**Summary & final evaluation** totals evaluated lessons, completed tasks, presentations, and lesson-star wins for every group, alongside editable final completion/presentation and notes. Game-star totals remain read-only live counts across all games created by current group members, separate from lesson-star awards. Refresh to fetch current counts. Historical groups and their evaluations remain visible even after their members move to other groups or courses.
...
User management displays separate editable display-name and username/email columns, plus a separate join date. Login identifiers are trimmed, lowercased, and unique. The administrator account is protected. Deleting a regular user requires confirmation and transactionally removes their games and associated database records, comments, stars, memberships, and sessions. Shared studios and course/group evaluations are preserved. Existing generated files may remain on disk, but deleted games are no longer served by the app.
